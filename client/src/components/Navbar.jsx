...
const Navbar = () => {
  ...
  const dropdownStyle = { position: 'absolute', top: '100%', left: '0', backgroundColor: 'white', boxShadow: '0 8px 16px rgba(0,0,0,0.1)', borderRadius: '8px', padding: '10px 0', minWidth: '220px', display: showDropdown ? 'block' : 'none', zIndex: '1001', border: '1px solid #eee' };
      ...
      <div style={{ display: 'flex', alignItems: 'center' }}>
        ...
        {token ? (
          <>
            ...
            {isAdmin && (
              <Link style={{ ...linkStyle, color: '#f39c12', fontWeight: 'bold' }} to="/admin?tab=appointments">Admin Panel</Link>
            )}
            {isDoctor && (
              <Link style={{ ...linkStyle, color: '#3498db', fontWeight: 'bold' }} to="/doctor-dashboard?tab=appointments">Doctor Panel</Link>
            )}
